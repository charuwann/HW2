...
<?PHP
	...
	$query = "SELECT password FROM A_LOGIN WHERE id='$id'";
	$parseRequest1 = oci_parse($conn, $query);
	...
	if($old_password == $oldpassword)
	{
		...
		{
		$query = "UPDATE A_LOGIN SET password ='$newpassword' WHERE
		password ='$oldpassword' and id='$id'";
		$parseRequest = oci_parse($conn, $query);
		oci_execute($parseRequest);
		
		die( '<script>window.location = "MemberPage2.php";</script>');
		}
...
<form action='changpassword2.php' method='post'>
